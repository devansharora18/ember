...
class _ReaderScreenState extends ConsumerState<ReaderScreen> {
  ...
  int _charsPerPage(BuildContext context) {
    ...
    final textPad = const EdgeInsets.fromLTRB(24, 48, 24, 4);
    final width = size.width - textPad.left - textPad.right;
    final height = size.height - pad.top - pad.bottom - textPad.top - textPad.bottom;
    if (width <= 0 || height <= 0) return 1000;

    final tp = TextPainter(
      text: TextSpan(text: 'X', style: _textStyle(height: 1.7)),
      textDirection: TextDirection.ltr,
    )..layout(maxWidth: width);

    final cpl = (width / tp.width).floor().clamp(1, 999);
    final lpp = (height / tp.height).floor().clamp(1, 999);
    return cpl * lpp;
  }
        ...
        child: Column(
          ...
          children: [
            Padding(
              ...
              child: Text('Contents', style: GoogleFonts.inter(color: _darkMode ? Colors.white : Colors.black87, fontSize: 16, fontWeight: FontWeight.w600)),
            ),
          ...
          child: Column(
            ...
            children: [
              Padding(
                ...
                child: Text('Select font', style: GoogleFonts.inter(color: _darkMode ? Colors.white : Colors.black87, fontSize: 15, fontWeight: FontWeight.w600)),
              ),
              ...fonts.map((f) => ListTile(
                ...
                onTap: () {
                  ...
                  BookStorage.saveFontFamily(widget.book.filePath, f);
                  Navigator.pop(ctx);
                },
              )),
            ],
      ...
      child: Stack(
        children: [
          PageView.builder(
            ...
            onPageChanged: (page) {
              if (page == 0 && coverCount > 0) { _currentPage = 0; return; }
              _onPageChanged(page - coverCount);
            },
          ...
          AnimatedOpacity(
            ...
            child: IgnorePointer(ignoring: !_controlsVisible, child: _buildAppBar(book, pad)),
          ),
  ...
  Widget _buildAppBar(Book book, EdgeInsets pad) {
    ...
    final bg = _darkMode ? const Color(0xDD000000) : const Color(0xDDF5F5F0);
    return Container(
        ...
        child: Row(
          children: [
            ...
            PopupMenuButton<String>(
              ...
              itemBuilder: (_) => [
                _menuPopItem('Select font…', 'select_font', false),
                _menuPopItem(_darkMode ? 'Light mode' : 'Dark mode', 'toggle_mode', false),
              ],
            ),
          ],
        ),
  ...
  PopupMenuItem<String> _menuPopItem(String label, String value, bool selected) {
    return PopupMenuItem<String>(
      value: value,
      height: 38,
      child: Text(label, style: GoogleFonts.inter(color: selected ? (_darkMode ? Colors.white : Colors.black87) : (_darkMode ? const Color(0xFFAAAAAA) : const Color(0xFF666666)), fontSize: 13, fontWeight: selected ? FontWeight.w600 : FontWeight.w400)),
    );
  }
        ...
        child: Padding(
          ...
          child: Row(
            children: [
              ...
              const Spacer(),
              Text('${_fontSize.round()}', style: GoogleFonts.inter(color: fg, fontSize: 11)),
            ],
          ),
        ),
      ),
